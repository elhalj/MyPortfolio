...
export default function BlogPostPage({ params }: { params: { slug: string } }) {
  ...
  const post = Array.isArray(blog)
    ? blog.find(
        (p) =>
          String(p._id) === String(slug) || String(p.slug) === String(slug),
      )
    : null;
  ...
  if (!post) {
    return (
      <div className="flex h-screen flex-col items-center justify-center text-white">
        ...
        <Link
          href="/blog"
          className="text-white px-2 py-1 rounded-full bg-amber-600"
        >
          Retour au blog
        </Link>
      </div>
  ...
  return (
    <ScrollAnimation animation="fade-right" className="delay-500">
      <div className="min-h-screen py-10 px-4 bg-linear-to-b from-gray-900 via-gray-800 to-gray-900 text-white font-[cormorant]">
        <div className="max-w-4xl mx-auto bg-gray-800 p-8 rounded-lg shadow-lg">
          ...
          <div className="flex gap-2 mb-4">
            {post.etat && (
              <span className="text-xs px-2 py-1 rounded-full bg-emerald-600">
                {post.etat}
              </span>
            )}
            {post.date && (
